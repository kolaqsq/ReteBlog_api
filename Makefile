...
php-reindex-books:
    docker-compose exec skprods-php-fpm php artisan reindex:books

### Команды для работы с контейнерами приложения
exec-php-fpm: # заходим в контейнер с php
	docker-compose exec skprods-php-fpm bash
exec-nginx: # заходим в контейнер с nginx
	docker-compose exec skprods-nginx bash
exec-redis: # заходим в контейнер с redis
	docker-compose exec skprods-redis bash
exec-percona: # заходим в контейнер с redis
	docker-compose exec skprods-percona bash

app-supervisor-start: # запуск supervisor для очередей
	docker-compose exec -u root skprods-php-fpm /usr/sbin/service supervisor start
app-supervisor-stop: # остановка supervisor
	docker-compose exec -u root skprods-php-fpm /usr/sbin/service supervisor stop
app-supervisor-restart: # рестарт supervisor
	docker-compose exec -u root skprods-php-fpm /usr/sbin/service supervisor restart
app-supervisor-status: # статус supervisor
	docker-compose exec -u root skprods-php-fpm /usr/sbin/service supervisor status
...
load-vendor:
	rm -rf ./vendor
	docker cp skprods-php-fpm:/var/www/vendor/. ./vendor
load-vendor-dev:
	rm -rf ./vendor
	docker cp skprods-php-fpm-dev:/var/www/vendor/. ./vendor

redis-config:
	docker-compose exec skprods-redis redis-cli config set stop-writes-on-bgsave-error no
	docker-compose exec skprods-redis redis-cli config get stop-writes-on-bgsave-error
